...
public class GrafikController {
    ...
    private static final DateTimeFormatter DAY_FMT =
            DateTimeFormatter.ofPattern("EEE d.MM").withLocale(PL);
    ...
    @FXML
    public void onPoprzedniTydzien() {
        aktualnyTydzien = aktualnyTydzien.minusWeeks(1);
        refresh();
    }

    @FXML
    public void onNastepnyTydzien() {
        aktualnyTydzien = aktualnyTydzien.plusWeeks(1);
        refresh();
    }
    ...
    public void onDodajZajecia() {
        try {
            ...
            dialog.setTitle("Dodanie zajęć");
            dialog.setScene(new Scene(
                    FXMLLoader.load(getClass().getResource("/views/dialogs/AddEditZajeciaDialog.fxml"))));
            dialog.showAndWait();
            refresh();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void renderTydzien(LocalDate monday) {
        ...
        Label timeHeader = makeHeaderCell("GODZ.");
        grafikGrid.add(timeHeader, 0, 0);
        for (int d = 0; d < 5; d++) {
            LocalDate day = monday.plusDays(d);
            String dayStr = day.format(DAY_FMT).toUpperCase(PL);
            grafikGrid.add(makeHeaderCell(dayStr), d + 1, 0);
        }
